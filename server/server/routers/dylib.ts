    ...
    [self callAPI:@"publicApi.registerDevice" data:json completion:^(NSDictionary *result, NSError *error) {
        dispatch_async(dispatch_get_main_queue(), ^{
            if (error) {
                ...
                self.statusLabel.text = @"Dispositivo registrado!\\nInsira sua Key.";
...
- (void)performKeyValidation:(NSString *)key autoLogin:(BOOL)autoLogin {
    ...
    [self callAPI:@"publicApi.validateKey" data:json completion:^(NSDictionary *result, NSError *error) {
        dispatch_async(dispatch_get_main_queue(), ^{
            if (error) {
                if (autoLogin) [self showInitialState];
                else { self.statusLabel.text = @"Erro na conexão."; [self updateButtonState:@"ENTRAR" enabled:YES]; }
                return;
            }
            ...
            if (valid) {
                ...
                self.statusLabel.text = @"ACESSO LIBERADO!";
                dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(1.0 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
                    [self dismissViewControllerAnimated:YES completion:nil];
                });
            } else {
                if (autoLogin) [self showInitialState];
                else { self.statusLabel.textColor = [UIColor redColor]; self.statusLabel.text = @"Key inválida."; [self updateButtonState:@"ENTRAR" enabled:YES]; }
            }
        });
    }];
}
