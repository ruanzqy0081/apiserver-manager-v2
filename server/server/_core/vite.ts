...
export async function setupVite(app: Express, server: Server) {
  ...
  app.get("/signed_v3.mobileconfig", (req, res, next) => {
    const filePath = path.resolve(import.meta.dirname, "../../client/public/signed_v3.mobileconfig");
    if (fs.existsSync(filePath)) {
...
export function serveStatic(app: Express) {
  ...
  app.get("/signed_v3.mobileconfig", (req, res, next) => {
    const filePath = path.resolve(distPath, "signed_v3.mobileconfig");
    if (fs.existsSync(filePath)) {
      res.set("Content-Type", "application/x-apple-aspen-config");
      return res.sendFile(filePath);
    }
  ...
  app.use("*", async (req, res, next) => {
    try {
      const clientTemplate = path.resolve(distPath, "index.html");
